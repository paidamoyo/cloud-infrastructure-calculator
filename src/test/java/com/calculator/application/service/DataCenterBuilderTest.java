...
public class DataCenterBuilderTest {

    @Test
    public void shouldCreateDataCentersFromHost() throws Exception {
        ...
        List<Host> hosts = Arrays.asList(hostTwo, hostFive, hostSeven, hostNine, hostThree, hostTen, hostSix,
                hostEight);
        //when
        List<DataCenter> dataCenters = new DataCenterBuilder(hosts).create();

        //then
        DataCenter dataCenterZero = DataCenter.from("0", Arrays.asList(hostTwo, hostFive, hostSeven));
        DataCenter dataCenterOne = DataCenter.from("1", Arrays.asList(hostNine, hostThree));
        DataCenter dataCenterTwo = DataCenter.from("2", Arrays.asList(hostTen, hostSix, hostEight));
        assertThat(dataCenters, is(Arrays.asList(dataCenterZero, dataCenterOne, dataCenterTwo)));
    }
}